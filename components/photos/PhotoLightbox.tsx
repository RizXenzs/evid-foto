...
import { Download, Calendar, MapPin, Tag, User, FolderOpen, X, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function PhotoLightbox({ photos, initialIndex, onClose, onDownload }: PhotoLightboxProps) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={next}
        className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl bg-white/10 hover:bg-white/20
          flex items-center justify-center text-white transition-all z-10 disabled:opacity-30"
        style={{ right: '320px' }}
        disabled={photos.length <= 1}
      >
      ...
      {/* Info panel */}
      <div className="w-80 h-full flex flex-col border-l border-white/10 overflow-y-auto"
